...
CGINCLUDE 
	...
	v2f vert(appdata_full v)
	{
		...
		// Calculate the grab pass position for refraction
	...
	{ 
		...
		float2 distortion = (i.bumpCoords.xy * 0.5 + i.bumpCoords.zw * 0.5) * _RefractionStrength;
		float4 grabPos = i.grabPassPos;
		grabPos.xy += distortion;
		half4 refractionColor = tex2Dproj(_RefractionTex, UNITY_PROJ_COORD(grabPos));
		
		// Blend refraction with base color based on alpha
		baseColor.rgb = lerp(refractionColor.rgb, baseColor.rgb, baseColor.a);
